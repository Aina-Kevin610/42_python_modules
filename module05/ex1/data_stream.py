    ...
    def ingest(
        ...
        try:
            ...
            if isinstance(data, list):
                for x in data:
                    self.stock.append(
                        (self.process_count, str(x))
                    )
                    self.process_count += 1
            else:
                self.stock.append(
                    (self.process_count, str(data))
                )
                self.process_count += 1
    ...
    def ingest(
        ...
        try:
            ...
            if isinstance(data, list):
                for x in data:
                    self.stock.append(
                        (self.process_count, x)
                    )
                    self.process_count += 1
            else:
                self.stock.append(
                    (self.process_count, data)
                )
                self.process_count += 1
    ...
    def print_processors_stats(self) -> None:
        ...
        for x in self.proc:
            if x.name == "numeric_processor":
                print(
                    "Numeric Processor: "
                    f"total {self.stat.num} "
                    "items processed, "
                    f"remaining {len(x.stock)} "
                    "on processor"
                )
...
def main() -> None:
    ...
    print(
        "send first batch to data on stream:",
        stream_data,
        "\n",
    )
