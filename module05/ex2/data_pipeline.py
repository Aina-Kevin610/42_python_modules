...
class DataProcessor(ABC):
    ...
    def output(self) -> tuple[int, str]:
        return tuple(self.process_count, self.stock.pop(0))
...
class LogProcessor(DataProcessor):
    ...
    def ingest(self, data: dict[str, str] | list[dict[str | int]]) -> None:
        try:
            if not self.validate(data):
...
def main() -> None:
    ...
    print("Send first batch of data on stream: ", stream_data, "\n")
    ...
    print("\nSend another batch of data: ", stream_data, "\n")
    stream.process_stream(stream_data)
    stream.print_processors_stats()
    print()
